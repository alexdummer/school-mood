...
def save_session(phase, gut_count, mittel_count, schlecht_count, timestamp=None):
    ...
    with conn.session as s:
        s.execute(
            text("INSERT INTO session_logs (school_id, phase, gut_count, mittel_count, schlecht_count, timestamp) VALUES (:school, :phase, :g, :m, :s, :t)"), 
            {"school": school_id, "phase": phase, "g": gut_count, "m": mittel_count, "s": schlecht_count, "t": timestamp}
        )
        s.commit()
...
def get_data():
    ...
    school_id = st.session_state.get('school_id', 'default_school')
    try:
        df = conn.query("SELECT * FROM session_logs WHERE school_id = :school", params={"school": school_id})
    except Exception:
        df = pd.DataFrame()
...
def show_kiosk_active(phase):
    ...
    if st.session_state.voted:
        st.success("Danke! Deine Stimmung wurde gemerkt.")
        time.sleep(2)
        st.session_state.voted = False
        st.rerun()
    ...
    with st.expander("Lehrkraft-Bereich"):
        st.write(f"Bisherige Stimmen (werden beim Beenden in Cloud gespeichert):")
...
def show_admin_dashboard():
    ...
    if df.empty:
        st.warning("Noch keine Daten vorhanden oder keine Cloud-Verbindung.")
        return
